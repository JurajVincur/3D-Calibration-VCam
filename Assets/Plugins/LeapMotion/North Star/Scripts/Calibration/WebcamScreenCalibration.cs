    ...
    public class WebcamScreenCalibration : MonoBehaviour, IRuntimeGizmoComponent
    {
        ...
        public struct CalibrationDevice
        {
            ...
            public bool isConnected
            {
                get
                {
                    if (webcam != null && webcam.virtualCamera != null)
                    {
                        return webcam.virtualCamera.CameraImage != null && webcam.leftImage != null;
                    }
                    return false;
                }
            }
        ...
        private void OnEnable()
        {
            ...
            {
                calibrationDevices[i].initializeFields(calibrationDevices[i].webcam.virtualCamera.FrameWidth >> 1, calibrationDevices[i].webcam.virtualCamera.FrameHeight);
            }
        ...
        protected void Update()
        {
            ...
            for (int j = 0; j < calibrationDevices.Length; j++)
            {
                ...
                for (int i = 0; i < 2; i++)
                {
                    ...
                    if (optimizers[i].solver == null)
                    {
                        calibrationDevices[j].webcam.updateScreen(workingImage2, i == 0);
                    }
                }
        ...
        IEnumerator CalibrateScreenLocation() //TODO
        {
            //Reset some stuff
            HyperMegaStuff.HyperMegaLines.drawer.dontClear = false;
            monitorPattern.SetActive(false);
            ...
            for (int i = 0; i < calibrationDevices.Length; i++)
            {
                yield return StartCoroutine(calibrationDevices[i].webcam.virtualCamera.Read());
            }
            yield return null;
            ...
            for (int i = 0; i < _realDots.Count; i++)
            {
                ...
                while (foundDots == 0)
                {
                    for (int j = 0; j < calibrationDevices.Length; j++)
                    {
                        yield return StartCoroutine(calibrationDevices[j].webcam.virtualCamera.Read());
                    }
                    yield return null;
                    foundDots = 0;
                    for (int j = 0; j < calibrationDevices.Length; j++)
            ...
            for (int i = 0; i < 2; i++)
            {
                ...
                if (calibrationDevices[j].subtractionImage[i] != null)
                {
                    ...
                    if (blobs.Length > 0)
                    {
                        ...
                        if (drawer != null)
                        {
                            drawer.color = (j == 0) ? Color.cyan : Color.red;
        ...
        void calculateImageMetrics(int j)
        {
            ...
            for (int i = 0; i < 2; i++)
            {
                ...
                if (calibrationDevices[j].maskImage[i] != null)
                {
                    ...
                    Cv2.Threshold(workingImage, workingImage, 200, 1, ThresholdTypes.Binary);
                    double pixelSum = -workingImage.Sum().Val0; //TODO!!!!!!!!!!!!!!
